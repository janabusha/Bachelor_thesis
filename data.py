...
def allData(
):  # izveido katrā(Izpētes un kontroles) mapē txt failu (_Dati_apstradei.txt) ar visiem datiem no failiem all_samples.kraken2.txt
    group = 'Izpetes_grupa'
...
def getBacteria(
        group):  # no liela txt faila _Dati_apstradei.txt tiek atlasītas bakterijas (kārtas/order) un tiek izveidots fails Apkopotas_Bakterijas(karta).txt katrā no mapēm
...
def createTable(
        group):  # katra bakterija no faila Apkopotas_Bakterijas iziet cauri failam  Dati_apstradei un pieraksta kuram pacientam un cik daudz bija ši bakterija. Tiek izveidots fails Visi_dati.csv
...
def groupingData(group):  # ši metode atgriež datus lasamā veidā un saglabājās gala faila _Apkopotie_dati.csv

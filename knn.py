...
for seed in random_seed:
    ...
    for scor in ['roc_auc','accuracy']: # precizitātes mērs pēc kura tiek pielāgoti hiperparametri
        model = KNeighborsClassifier() # modeļa izveidošana
        n_neighbors = range(1, 21, 2) # iespējamais kaimiņu skaits
        weights = ['uniform', 'distance'] # iespējamo svaru veids
        metric = ['euclidean', 'manhattan', 'minkowski'] # iespējamie attāluma mēri

        grid = dict(n_neighbors=n_neighbors, weights=weights, metric=metric)
        cv = RepeatedStratifiedKFold(n_splits=5, n_repeats=2, random_state=1)  #atkārtota noslāņota šķērsvalidācija
        grid_search = GridSearchCV(estimator=model, param_grid=grid, n_jobs=-1, cv=cv, scoring=scor, error_score=0) # GridSearch() metode hiperparametru pielagošanai
